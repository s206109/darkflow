...
for dInd in np.arange(0,len(predBoxes)): #dInd = 何ファイル目なのかの数
    for pInd in np.arange(1,len(predBoxes[dInd])): #1つ目はファイル名なので。物体の数だけまわす
        ...
        resultDF = resultDF.append(pd.Series([np.max(ious),
                           ...
                           gtBox[maxInd].alpha, (predBox.z   - gtBox[maxInd].z) , predBox.filenum],
...
#plt.plot(['1:[-3pi/4<]','2:[-3pi/4:-pi/4]','3:[-pi/4:pi/4]','4:pi/4:3pi/4]','5:[<3pi/4]'],[error1, error2, error3, error4, error5],label = 'estimation with 2.5D anchor')
#plt.plot(['1:[-3pi/4<]','2:[-3pi/4:-pi/4]','3:[-pi/4:pi/4]','4:pi/4:3pi/4]','5:[<3pi/4]'],[bunsan1, bunsan2, bunsan3, bunsan4, bunsan5],label = 'bunsan with 2.5D anchor')
...
print(std_error)
plt.errorbar(['[0-10]','[10-20]','[20-30]','[30-40]','[40 over]'], [error10, error20, error30, error40, error40over], yerr=std_error, xerr=None,fmt='ro', ecolor='')
